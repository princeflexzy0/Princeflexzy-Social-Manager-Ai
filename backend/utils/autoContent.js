...
async function fetchTrendingViaAI(topic) {
  try {
    ...
    const prompt = `Today is ${today}. What is a realistic, current talking point or recent news about "${topic}" that someone in Nigeria or the UK might be discussing right now in ${new Date().getFullYear()}? Give me one short specific fact or storyline in 1-2 sentences. IMPORTANT: Do NOT reference events from before 2025. Do NOT mention any completed league seasons as if they are ongoing. If the EPL or Champions League season is over, talk about transfers, manager news, or pre-season instead. Be current and honest.`;
...
TODAY'S DATE: ${today}
IMPORTANT: You must only reference events, news and storylines that are current as of ${today}. The Premier League 2024/25 season has ENDED. Do NOT tweet as if matches are still being played this season. Instead talk about transfers, summer window, pre-season, manager changes, or next season hype.
...
`;
  ...
  const fallbacks = [
    'Man United need to sort out this transfer window. I said what I said.',
