...
exports.floatToHex = floatToHex;
exports.handleXMigration = handleXMigration;
exports.isOdd = isOdd;
/**
...
async function handleXMigration() {
    ...
    if (!response.ok) {
        throw new errors_js_1.XHomePageFetchError(response.status, response.statusText);
    }
    const htmlText = await response.text();
    // Parse HTML using linkedom
    let dom = (0, linkedom_1.parseHTML)(htmlText);
    ...
    if (migrationRedirectionUrl) {
        ...
        if (!redirectResponse.ok) {
            throw new errors_js_1.XMigrationRedirectionError(redirectResponse.status, redirectResponse.statusText);
        }
        const redirectHtml = await redirectResponse.text();
        dom = (0, linkedom_1.parseHTML)(redirectHtml);
    ...
    if (migrationForm) {
        ...
        if (!formResponse.ok) {
            throw new errors_js_1.XMigrationFormError(formResponse.status, formResponse.statusText);
        }
        const formHtml = await formResponse.text();
        dom = (0, linkedom_1.parseHTML)(formHtml);
        document = dom.window.document;
    }
